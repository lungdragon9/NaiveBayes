...
public class NaiveBayes extends Classifier 
  ...
  /**
   * weight for predictions James Stamps Edit
   */
  ...
  {
	  this.weight = new double[weights.length];
  ...
  throws Exception { 
	...
	for (int j = 0; j < m_NumClasses; j++) {
	  ...
	  //I belive make edit on this line. Add weight here
	  probs[j] *= temp;
	  probs[j] = Math.pow(probs[j], this.weight[attIndex]);
	  if (probs[j] > max) {
	    max = probs[j];
	  }
